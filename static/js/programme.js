...
/**
 * Vérifie que le fichier fourni dans le champ #csvEleves
 * fournit bien une liste d'élèves comme on l'attend
 * @param url l'URL de base du service web
 **/
function checkEleves(url){
    url=""+url; // make a copy to prevent the modification of document.location
    url=url.replace('index','')+'check_eleves';
    var q = $("#csvEleves").val();
    $.getJSON(
	url,// service de vérification des listes d'élèves
	{ q: q})      
	.done(function(data){                    // rappel en cas de réussite
            if (data!="OK"){
		alert(data);
		$("#csvEleves").val(""); // remise à blanc du nom de fichier
	    } else {
		var feedback=$("#feedback")
		feedback.text(q+" : OK");
		feedback.fadeIn("fast", function(){$(this).fadeOut(2000);});
	    }
	})
	.fail(function(){                        // rappel en cas d'échec
            alert("Échec de la communication de vérification");
	    $("#csvEleves").val(""); // remise à blanc du nom de fichier
	});
    return
}
...
/**
 * Vérifie que le fichier fourni dans le champ #csvElevesGroupes
 * fournit bien une liste de groupes comme on l'attend
 * @param url l'URL de base du service web
 **/
function checkGroupes(url){
    url=""+url; // make a copy to prevent the modification of document.location
    url=url.replace('index','')+'check_groupes';
    var q = $("#csvGroupes").val();
    $.getJSON(
	url,// service de vérification des listes d'élèves
	{ q: q})      
	.done(function(data){                    // rappel en cas de réussite
            if (data!="OK"){
		alert(data);
		$("#csvGroupes").val(""); // remise à blanc du nom de fichier
	    } else {
		var feedback=$("#feedback")
		feedback.text(q+" : OK");
		feedback.fadeIn("fast", function(){$(this).fadeOut(2000);});
